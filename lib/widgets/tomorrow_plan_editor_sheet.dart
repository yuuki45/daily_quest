...
class TomorrowPlanEditorSheet extends ConsumerStatefulWidget {
  const TomorrowPlanEditorSheet({super.key});

  @override
  ConsumerState<TomorrowPlanEditorSheet> createState() => _TomorrowPlanEditorSheetState();
}

class _TomorrowPlanEditorSheetState extends ConsumerState<TomorrowPlanEditorSheet> {
  ...
  void initState() {
    super.initState();
    // メインタスクがあるかチェック
    final plan = ref.read(tomorrowPlanNotifierProvider);
  ...
  Future<void> _addNewTask() async {
    ...
    // 明日のプランに追加
    if (isMain) {
      await ref.read(tomorrowPlanNotifierProvider.notifier).setMainTask(newTaskId);
    } else {
      await ref.read(tomorrowPlanNotifierProvider.notifier).addSubTask(newTaskId);
    }
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                  ...
                  children: [
                    ...
                    if (!_isMainTask)
                      Container(
                        ...
                        child: Text(
                          'あと${AppConstants.maxSubTasks - ref.watch(tomorrowPlanNotifierProvider).subTaskIds.length}つ追加可能',
                          style: TextStyle(
                            fontSize: 11,
                            fontWeight: FontWeight.w600,
                  ...
                  children: [
                    Expanded(
                      child: ChoiceChip(
                        ...
                        onSelected: ref.watch(tomorrowPlanNotifierProvider).mainTaskId == null
                            ? (selected) {
                                setState(() {
                                  _isMainTask = true;
                                });
                ...
                // 説明テキスト
                if (ref.watch(tomorrowPlanNotifierProvider).mainTaskId != null)
                  Padding(
